...
export class LoyaltyClient {
    ...
    constructor(
        ...
        baseUrl = "https://loyalty.yowyob.com",
        options: LoyaltyClientOptions = {}
    ) {
    ...
    async trackEvent(
        ...
        return this.request<EventProcessingResult>(
            "POST",
            `/api/v1/apps/${encodeURIComponent(this.publicKey)}/events`,
            {
                eventType,
                memberId,
    ...
    async getMemberPoints(memberId: string): Promise<Record<string, unknown>> {
        return this.request("GET", `/api/v1/members/${encodeURIComponent(memberId)}/points`);
    }

    /** Palier de fidélité courant d'un membre. */
    async getMemberTier(memberId: string): Promise<Record<string, unknown>> {
    ...
    async getWallet(memberId: string): Promise<Record<string, unknown>> {
    ...
    checkCallbackIntegrity(
        ...
        toleranceSeconds = LoyaltyClient.DEFAULT_SIGNATURE_TOLERANCE_SECONDS
    ): WebhookEvent {
        if (!this.webhookSecret) {
            throw new LoyaltyError(
                "webhookSecret non configuré : passez-le dans les options du constructeur de LoyaltyClient."
            );
        }
        ...
        const expected =
            "sha256=" + createHmac("sha256", this.webhookSecret).update(`${timestamp}.${body}`).digest("hex");
    ...
    private async request<T = Record<string, unknown>>(
        ...
        try {
            ...
            throw new ApiError(`Erreur réseau vers l'API Loyalty : ${(cause as Error).message}`, 0);
        }
        ...
        if (response.status === 401 || response.status === 403) {
            throw new AuthenticationError(
                `Clé API invalide ou révoquée (HTTP ${response.status}).`,
                response.status,
                decoded
            );
        }
        if (!response.ok) {
            const detail =
                (decoded as { detail?: string; title?: string } | null)?.detail ??
                (decoded as { title?: string } | null)?.title ??
